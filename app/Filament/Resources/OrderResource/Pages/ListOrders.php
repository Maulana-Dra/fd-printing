    ...
    public function getTabs(): array
    {
        ...
        return [
            ...
            'pending_payment' => Tab::make('Menunggu Bayar')
                ...
                ->badgeColor('danger')
                ->modifyQueryUsing(fn (Builder $q) => $q->where('status', OrderStatus::PENDING_PAYMENT->value)),

            'paid' => Tab::make('Sudah Dibayar')
                ->badge($counts[OrderStatus::PAID->value] ?? 0)
                ->badgeColor('info')
                ->modifyQueryUsing(fn (Builder $q) => $q->where('status', OrderStatus::PAID->value)),

            'processing' => Tab::make('Produksi')
                ->badge($counts[OrderStatus::PROCESSING->value] ?? 0)
                ->badgeColor('primary')
                ->modifyQueryUsing(fn (Builder $q) => $q->where('status', OrderStatus::PROCESSING->value)),

            'ready' => Tab::make('Siap Kirim')
                ->badge($counts[OrderStatus::READY->value] ?? 0)
                ->badgeColor('success')
                ->modifyQueryUsing(fn (Builder $q) => $q->where('status', OrderStatus::READY->value)),

            'shipped' => Tab::make('Dikirim')
                ->badge($counts[OrderStatus::SHIPPED->value] ?? 0)
                ->modifyQueryUsing(fn (Builder $q) => $q->where('status', OrderStatus::SHIPPED->value)),

            'done' => Tab::make('Selesai')
                ->badge($counts[OrderStatus::DONE->value] ?? 0)
                ->badgeColor('success')
                ->modifyQueryUsing(fn (Builder $q) => $q->where('status', OrderStatus::DONE->value)),
